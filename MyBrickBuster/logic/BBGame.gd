...
@onready var current_score_label = $Board/UI_header/score
@onready var high_score_label = $Board/UI_header/score_max
...
func save():
	...
	global.save_game.open("user://savegame.save", FileAccess.WRITE)
	
	# Store the save dictionary as a new line in the save file.
	global.save_game.store_line(JSON.stringify(save_dict))
...
func launch_line_calc():
	...
	#if line_direction.normalized().x == 1:
...
func launch_balls(direction = line_direction.normalized(), amount = ammo):
	...
	for i in amount:
		...
		add_child(next_ball)
		next_ball.connect("ball_no_contact_timeout", on_ball_no_contact_timeout)
		next_ball.connect("ball_died", on_ball_died)
...
func new_destroyable(destroyable_request, game_mode = "standard"):
	if destroyable_request.get_class():
		...
		if request_type == "BrickRequest":
			...
			next_destroyable.connect("brick_killed", on_destroyable_killed)
			...
			next_destroyable.connect("special_area_entered", on_special_area_entered)
			next_destroyable.connect("special_killed", on_destroyable_killed)
		...
		print(destroyable_request.column_vert_point)
		...
		print(next_destroyable.get_position())	
...
func update_score_labels():
	...
	high_score_label.text = str(score_max)
...
func reset():
	#$AnimationPlayer.play("fadeout/fadeout")
...
func _ready():
	...
	#meta_area.set("pause_mode", Node.PROCESS_MODE_ALWAYS)
	#meta_area.connect("pause_menu_toggled", on_pause_menu_toggled)
	#meta_area.connect("restart_button_clicked", on_restart_button_clicked)
	#meta_area.connect("quit_to_menu_button_clicked", on_quit_to_menu_button_clicked)
	
	launch_line.add_point(Vector2(0,0), 0)
	launch_line.add_point(Vector2(0,0), 1)
	launch_line.default_color = Color(0xffffff)
	...
	var selected_game_mode = "standard"
	...
	var f = FileAccess.open("user://savegame.save", FileAccess.READ)
	if (f):
		score_max = f.get_var()
		update_score_labels()
	...
	#	meta_area.help_popup.connect("visibility_changed", on_help_menu_visibility_changed)
