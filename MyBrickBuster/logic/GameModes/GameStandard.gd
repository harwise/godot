...
func new_destroyable_line(health, vert_point = 0):
	...
	rng.randomize()
	if !free_columns.is_empty(): #&& rng.randi_range(0, 4) == 4:
...
func on_ball_no_contact_timeout(ball_position, ball_linear_velocity):
	...
	params.position = game_control.columns[3].get_point_position(line_point)
	var things_at_point = get_world_2d().direct_space_state.intersect_point(params)
	
	if line_point < 8 && things_at_point.is_empty():
		var bounce_request = game_control.SpecialRequest.new()
		bounce_request.column_vert_point = line_point
		bounce_request.column_num = 3
		bounce_request.mode = "bounce"
...
func _ready():
	game_control.connect("ball_no_contact_timeout", on_ball_no_contact_timeout)
	game_control.connect("reset_triggered", on_reset_triggered)
	game_control.connect("ball_died", on_ball_died)
...
func _process(delta):
	if !game_control.game_over:
		...
		if !game_control.live_balls.is_empty():
